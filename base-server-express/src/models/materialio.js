...
class MaterialIOModel {
    constructor() {
        this.sequelize = Connect.sequelize;
        this.model = this.sequelize.define('MaterialIO', {
            id: {
                field: 'id',
        ...
        return new Promise((fulfill, reject) => {
            this.model.max('id')
                .then(maxID => {
                    ...
                    let pk = Constant.MATERIAL_IO_TBL_SHORT_NAME + '_' + maxID;
                    fulfill(pk);
                }).catch(err => {
                    reject(err);
                })
        ...
        return new Promise(async (fulfill, reject) => {
            await asyncForEach(payload.dataList, async (item) => {
                ...
                await this.generatePK().then(async pk => {
                    item.pk = pk;
                    item.ioGroupPK = payload.ioGroupPK;
    ...
    updateMultiple(payload) {
        return new Promise(async (fulfill, reject) => {
            this.destroyByMaterialPK(payload.ioGroupPK).then(async (status) => {
                await asyncForEach(payload.dataList, async (item) => {
                    item.timeCreated = new Date().getTime();
                    item.timeModified = new Date().getTime();
                    item.createdBy = payload.userPK;
    ...
    getByMaterialPK(materialPK) {
        return new Promise((fulfill, reject) => {
            let sql = `select * from MaterialIO where ioGroupPK = '${materialPK}' order by timeModified desc`;
...
module.exports = new MaterialIOModel();